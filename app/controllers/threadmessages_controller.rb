class ThreadmessagesController < ApplicationController
  ...
  def index
    ...
    3.times { @threadmessage.assets.build }
  ...
  def create
    ...
    session[:list_threadmessage]=array_of_threadids
    respond_to do |format|
  ...
  def edit
    @threadmessage = Threadmessage.find(params[:id])
    count=3-@threadmessage.assets.count
    count.times { @threadmessage.assets.build }
    session[:list_threadmessage]=[@threadmessage.id]
  end
  ...
  def update
    ...
    respond_to do |format|
      if counter==1
        format.html { redirect_to threadmessages_path, notice: 'Threadmessage was successfully created.' }
